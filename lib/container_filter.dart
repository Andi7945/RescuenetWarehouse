...
class ContainerFilter {
  ...
  ContainerFilter(this.field, this.value) : fn = _calcFn(field);
...
bool Function(RescueContainer, Iterable<Item>) _containerFilterMatches(
  ...
  if (field == FilterField.all) {
    return (c, _) =>
        containerFilters.values.any((element) => _contains(element(c), value));
  }
  var containerFilterBuilder = containerFilters[field];
  if (containerFilterBuilder != null) {
    return (c, _) => _contains(containerFilterBuilder(c), value);
  }
  return (c, _) => false;
}
