  ...
  class ItemSortButton extends river.ConsumerWidget {
    ...
    Widget _btnChangeSortOrder(
        ItemSortingOption currentOption, Function(ItemSortingOption) change) {
      return Padding(
          padding: const EdgeInsets.only(right: 4),
          child: PopupMenuButton(
              itemBuilder: (ctx) => itemSortingOptions
                  .map((o) => _createOption(o, change))
                  .toList(),
              child: Row(children: [
    ...
    PopupMenuEntry _createOption(
            ItemSortingOption so, Function(ItemSortingOption) change) =>
        PopupMenuItem(
            value: so, onTap: () => change(so), child: Text(so.displayName));
  }
