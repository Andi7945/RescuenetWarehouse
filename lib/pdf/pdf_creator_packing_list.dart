...
Future<pw.Document> Function(PdfPageFormat) createPackingListPdf(
    ...
    (format) async {
      ...
      var page = await _singlePage(format, HeaderProvider(), list);
      pdf.addPage(page);
      return pdf;
    };

Future<pw.Page> _singlePage(
        PdfPageFormat format, HeaderProvider prov, PackingList list) async =>
    pageHeaderFooter(format, await prov.provideHeader(list), _body(list),
